...
public abstract class TileChunkLoaderBase extends TileEntity implements IChickenChunkLoader {
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        if (tag.hasKey("powered")) powered = tag.getBoolean("powered");
        loaded = true;
    }

    public void validate() {
        super.validate();
        if (!worldObj.isRemote && loaded && !powered) activate();

        if (worldObj.isRemote) renderInfo = new RenderInfo();
    }
    ...
    public static boolean isPoweringTo(World world, int x, int y, int z, int side) {
        return world.getBlock(x, y, z)
            .isProvidingWeakPower(world, x, y, z, side) > 0;
    }

    public void invalidate() {
        super.invalidate();
        if (!worldObj.isRemote) deactivate();
    }
    ...
    public void onBlockPlacedBy(EntityLivingBase entityliving) {
        if (entityliving instanceof EntityPlayer) owner = entityliving.getCommandSenderName();
