...
public class PlayerChunkViewer extends JFrame {
    ...
    public static class TicketInfo {
        ...
        public TicketInfo(PacketCustom packet, WorldClient world) {
            ...
            if (packet.readBoolean()) player = packet.readString();
    ...
    public class TicketInfoDialog extends JDialog implements LayoutManager {
        ...
        public TicketInfoDialog(LinkedList<TicketInfo> tickets) {
            ...
            for (TicketInfo ticket : tickets) {
                ...
                if (ticket.player != null) ident += ", " + ticket.player;
                ident += " #" + ticket.ID;
    ...
    protected void startUpdateThread() {
        new Thread("Info Frame Update Thread") {
            ...
            public void run() {
                while (true) {
                    if (Minecraft.getMinecraft().currentScreen instanceof GuiMainMenu) dispose();

                    if (instance == null || !isVisible()) return;
    ...
    protected void update() {
        ...
        if (needsReset) {
            ...
            if (dims.contains(selectedDim)) dimComboBox.setSelectedItem(selectedDim);
        }
        repaint();
    }
    ...
    public class DisplayArea extends JPanel implements MouseListener, MouseMotionListener {
        ...
        public void paint(Graphics g1) {
            synchronized (dimensionChunks) {
                ...
                for (TicketInfo ticket : dimInfo.tickets.values()) {
                    if (!ticket.chunkSet.isEmpty()) numTickets++;
                    for (ChunkCoordIntPair coord : ticket.chunkSet) forcedChunks.add(coord);
                }
        ...
        public void mouseClicked(MouseEvent event) {
            ...
            if (!mouseOverTickets.isEmpty()) new TicketInfoDialog(mouseOverTickets);
        }
            ...
            synchronized (dimensionChunks) {
                ...
                if (!mouseOverTickets.isEmpty()) {
                    ...
                    for (TicketInfo info : mouseOverTickets) {
                        tip += "\n" + info.modId;
                        if (info.player != null) tip += ", " + info.player;
                    }
                }
        ...
        public void setToolTipText(String paramString) {
            if (paramString == null) super.setToolTipText(null);
            else super.setToolTipText("<html>" + paramString.replace("\n", "<br>") + "</html>");
        }
    }
    ...
    public void handleChunkChange(int dimension, ChunkCoordIntPair coord, boolean add) {
        synchronized (dimensionChunks) {
            if (add) dimensionChunks.get(dimension).allchunks.add(coord);
            else dimensionChunks.get(dimension).allchunks.remove(coord);
        }
    }
    ...
    public void handleTicketChange(int dimension, int ticketID, ChunkCoordIntPair coord, boolean force) {
        synchronized (dimensionChunks) {
            ...
            if (force) ticket.chunkSet.add(coord);
            else ticket.chunkSet.remove(coord);
        }
    }
    ...
    public void handlePlayerUpdate(String username, int dimension, Vector3 position) {
        synchronized (dimensionChunks) {
            ...
            if (info == null) players.put(username, info = new PlayerInfo(username));
