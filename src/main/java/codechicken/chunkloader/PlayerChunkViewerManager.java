...
public class PlayerChunkViewerManager {
    ...
    public static PlayerChunkViewerManager instance() {
        if (instance == null) instance = new PlayerChunkViewerManager();
        return instance;
    }
    ...
    public void update() {
        ...
        for (DimensionChange change : dimChanges)
            if (change.add) for (PlayerChunkViewerTracker tracker : playerViewers) tracker.loadDimension(change.world);

        for (ChunkChange change : chunkChanges)
            for (PlayerChunkViewerTracker tracker : playerViewers) tracker.sendChunkChange(change);

        for (TicketChange change : ticketChanges) {
            if (ticketIDs.containsKey(change.ticket)) {
                for (PlayerChunkViewerTracker tracker : playerViewers) tracker.sendTicketChange(change);
            } else {
        ...
        for (DimensionChange change : dimChanges)
            if (!change.add) for (PlayerChunkViewerTracker tracker : playerViewers)
                tracker.unloadDimension(CommonUtils.getDimension(change.world));

        if (time % 10 == 0) {
            for (EntityPlayer player : ServerUtils.getPlayers())
                for (PlayerChunkViewerTracker tracker : playerViewers) tracker.updatePlayer(player);
        }
        ...
        for (String username : addViewers) {
            ...
            if (playerViewers.isEmpty()) updateChunkChangeMap();
    ...
    private void updateChunkChangeMap() {
        for (WorldServer world : DimensionManager.getWorlds()) {
            ...
            for (Chunk chunk : loadedChunkCopy) allChunks.add(chunk.getChunkCoordIntPair());
    ...
    public void calculateChunkChanges(WorldServer world) {
        ...
        for (Chunk chunk : loadedChunkCopy) {
            ...
            allChunks.add(coord);
            if (!wasLoadedChunks.remove(coord)) chunkChanges.add(new ChunkChange(dimension, coord, true));
        }

        for (ChunkCoordIntPair coord : wasLoadedChunks) chunkChanges.add(new ChunkChange(dimension, coord, false));
