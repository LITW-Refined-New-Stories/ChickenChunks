...
public class BlockChunkLoader extends BlockContainer {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7,
        ...
        if (!world.isRemote) {
            ...
            } else player.addChatMessage(new ChatComponentTranslation("chickenchunks.accessdenied"));
        }
        return true;
    }
    ...
    public TileEntity createNewTileEntity(World world, int meta) {
        if (meta == 0) return new TileChunkLoader();
        else if (meta == 1) return new TileSpotLoader();
        else return null;
    }
    ...
    public void registerBlockIcons(IIconRegister par1IconRegister) {
        icons = new IIcon[2][3];
        for (int m = 0; m < icons.length; m++) for (int i = 0; i < icons[m].length; i++)
