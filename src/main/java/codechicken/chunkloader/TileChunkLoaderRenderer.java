...
public class TileChunkLoaderRenderer extends TileEntitySpecialRenderer {
    ...
    public static class RenderInfo {
        ...
        public void update(TileChunkLoaderBase chunkLoader) {
            if (activationCounter < 20 && chunkLoader.active) activationCounter++;
            else if (activationCounter > 0 && !chunkLoader.active) activationCounter--;
        }
    }
    ...
    public void renderTileEntityAt(TileEntity tile, double d, double d1, double d2, float f) {
        ...
        if (chunkLoader.active && renderInfo.activationCounter < 20) active += f / 20D;
        else if (!chunkLoader.active && renderInfo.activationCounter > 0) active -= f / 20D;
    ...
    public Point2D.Double findIntersection(Line2D line1, Line2D line2) {
        ...
        if (ptOnLineInSegment(pt, line1) && ptOnLineInSegment(pt, line2)) return pt;
        ...
        for (ChunkCoordIntPair pair : chunkSet) {
            ...
            for (int side = 0; side < 4; side++) {
                ...
                for (int ray = 0; ray < 4; ray++) {
                    ...
                    if (lenPow2 > distances[ray]) distances[ray] = lenPow2;
                }
            }
        }
